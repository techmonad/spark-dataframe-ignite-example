...
object IgniteDataFrameWriteExample extends App {
  ...
  private val CONFIG = "/home/satendra/open-source/ignite-example/config/example-ignite.xml"
  ...
  def writeJSonToIgnite(implicit spark: SparkSession): Unit = {
    ...
    val personsDataFrame: DataFrame = spark.read.json(resolveIgnitePath("examples/src/main/resources/person.json").getAbsolutePath)
